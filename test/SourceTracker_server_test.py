...
class SourceTrackerTest(unittest.TestCase):
    ...
    # NOTE: According to Python unittest naming rules test method names should start from 'test'. # noqa
    def test_run_SourceTracker_ok(self):
        # call your implementation
        ret = self.serviceImpl.run_SourceTracker(self.ctx,
    ...
    def test_run_SourceTracker_min_len_negative(self):
        with self.assertRaisesRegex(ValueError, 'min_length parameter cannot be negative'):
            self.serviceImpl.run_SourceTracker(self.ctx,
                                              {'workspace_name': self.wsName,
                                               'assembly_input_ref': '1/fake/3',
                                               'min_length': '-10'})

    def test_run_SourceTracker_min_len_parse(self):
